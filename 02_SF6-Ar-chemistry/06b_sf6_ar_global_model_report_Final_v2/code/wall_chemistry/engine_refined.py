#!/usr/bin/env python3
"""
engine.py — Proven SF6/Ar global model solver
==============================================
Self-consistent 0D steady-state solver for 26-species SF6/Ar ICP plasma.
This is the validated solver from the reverse-engineering project, adapted
for the modular codebase. All physics is preserved exactly.

References:
  Lallement et al., PSST 18, 025001 (2009)
  Ryan & Plumb, Plasma Chem. Plasma Process. 10, 207 (1990)
  Velazco & Setser, J. Chem. Phys. 69, 4357 (1978)
"""
...
#
# Rate coefficients from Velazco & Setser (1975), Kolts & Setser (1978)
...
def solve_model(P_rf=1500, p_mTorr=10, frac_Ar=0., Q_sccm=40, Tgas=300, T_neg=0.3,
    ...
    """Solve with Penning ionization and Ar* quenching."""
    ...
    for outer in range(500):
        ...
        for sub in range(20):
            # SF6 balance: now includes Penning consumption of SF6
            ...
            nSF5 = (k['d1']*ne*nSF6+k['nr41']*nSF4*nF + k['qnch_SF6']*nArm*nSF6/(nSF6+1e-30)*0 + R_Penning*0)/((k['d7']+k['iz25']+k['iz26'])*ne+k['nr42']*nF+2*k['nr45']*nSF5+kwS+k['qnch_SFx']*nArm+1/tau+1e-30)
            # SF5 production from Penning: Ar* + SF6 -> SF5+ + F + Ar + e
            # But this produces SF5+, not neutral SF5. So no direct neutral SF5 from Penning.
            # However, non-ionizing quench: Ar* + SF6 -> Ar + SF5 + F produces neutral SF5
            ...
            # F balance: now includes F from Penning ionization and non-ionizing quench
            ...
            # NEW: F from Penning ionization (1 F per event) and non-ionizing quench (1 F)
            RF += k['Penn_SF6']*nArm*nSF6  # Ar* + SF6 -> SF5+ + F + Ar + e
            RF += k['qnch_SF6']*nArm*nSF6  # Ar* + SF6 -> Ar + SF5 + F
            # F from Ar*+F2 quenching (produces 2F)
