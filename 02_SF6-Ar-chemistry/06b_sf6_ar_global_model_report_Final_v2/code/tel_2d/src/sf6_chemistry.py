...
def compute_rates(Te, ng_cm3, frac_Ar=0.0):
    ...
    # === Electron-impact rate coefficients (Lallement/Kokkoris) ===
    # Arrhenius form k = A * exp(-E_th/Te) [m3/s converted to cm3/s where needed]
    # These rates are validated against Lallement et al. (2009) experimental data.
    # The Kokkoris Druyvesteyn 5-parameter polynomial form k=exp(A+B*ln(Te)+C/Te+D/Te^2+E/Te^3)
    # is equivalent but uses a different Te convention (2/3 of mean energy for Druyvesteyn EEDF).
    # At the Maxwellian Te ~ 2.5-3.5 eV used here, the Arrhenius fits give the same rates as 
    # evaluating the Kokkoris polynomial at the Druyvesteyn-equivalent Te ~ 4-5 eV.
    cm3 = 1e-6  # m3 to cm3 conversion
    
    # SF6 dissociation (G1-G5)
    k['d1'] = 1.5e-7 * np.exp(-8.1/Te) * cm3    # G1: SF6 → SF5 + F
    k['d2'] = 9e-9 * np.exp(-13.4/Te) * cm3      # G2: SF6 → SF4 + 2F
    k['d3'] = 2.5e-8 * np.exp(-33.5/Te) * cm3    # G3: SF6 → SF3 + 3F
    k['d4'] = 2.3e-8 * np.exp(-23.9/Te) * cm3    # G4: SF6 → SF2 + 2F + F2
    k['d5'] = 1.5e-9 * np.exp(-26.0/Te) * cm3    # G5: SF6 → SF + 3F + F2
    k['d6'] = 1.2e-8 * np.exp(-5.8/Te) * cm3     # G6: F2 → 2F
    
    # Sequential fragment dissociation (G7-G11, same cross-section as G1)
    ...
    # Dissociative ionization of SF6 (G8-G10 in Kokkoris numbering)
    k['iz18'] = 1.2e-7 * np.exp(-18.1/Te) * cm3  # SF6 → SF5+ + F + 2e
    k['iz19'] = 8.4e-9 * np.exp(-19.9/Te) * cm3  # SF6 → SF4+ + 2F + 2e
    k['iz20'] = 3.2e-8 * np.exp(-20.7/Te) * cm3  # SF6 → SF3+ + 3F + 2e
    ...
    # Fragment ionization (G11-G15)
    k['iz25'] = 1.0e-7 * np.exp(-17.8/Te) * cm3  # SF5 → SF5+
    k['iz26'] = 9.4e-8 * np.exp(-22.8/Te) * cm3  # SF5 → SF4+ + F
    k['iz27'] = 1.0e-7 * np.exp(-17.0/Te) * cm3  # SF3 → SF3+
    k['iz28'] = 1.6e-7 * np.exp(-18.6/Te) * cm3  # SF4 → SF4+
    k['iz29'] = 1.0e-7 * np.exp(-20.0/Te) * cm3  # S → S+
    
    # Dissociative attachment (G17-G18 from Kokkoris)
    # === Attachment ===
