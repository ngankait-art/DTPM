...
def gen_absolute_validation_4panel(power_data, out_dir):
    ...
    (a) Absolute [F] — Mettler Fig 4.5 overlay
    (b) Source-sink balance (normalised to 700 W)
    ...
    # Mettler reference cluster ~4e14 at 700 W (Fig 4.5)
    axes[0, 0].plot(700, 4e14, '^', ms=12, color='red', label='Mettler (700 W)')
    axes[0, 0].plot([500, 700, 900], [3.2e14, 4e14, 4.5e14], 'o', ms=8,
                    color='royalblue', alpha=0.8, label='Mettler Fig 4.5')
    axes[0, 0].set_xlabel('RF Power (W)')
    axes[0, 0].set_ylabel('[F] (cm$^{-3}$)')
    axes[0, 0].set_title('(a) Absolute [F] — Mettler Fig 4.5', fontsize=11, fontweight='bold')
    axes[0, 0].legend(fontsize=9)
    axes[0, 0].grid(True, alpha=0.3, which='both')

    # (b) Source-sink balance (normalised to 700 W)
    i700 = x.index(700) if 700 in x else len(x) // 2
    src_norm = [ne_icp[i] / max(ne_icp[i700], 1) for i in range(len(x))]
    F_norm = [F_icp[i] / max(F_icp[i700], 1) for i in range(len(x))]
    axes[0, 1].plot(x, src_norm, 'k-o', lw=2.5, ms=7, label='$n_e / n_{e,700}$')
    axes[0, 1].plot(x, F_norm, 'r-s', lw=2.5, ms=7, label='[F] / [F]$_{700}$')
    axes[0, 1].axhline(1.0, ls=':', color='gray', lw=1)
    axes[0, 1].set_xlabel('RF Power (W)')
    axes[0, 1].set_ylabel('Normalised to 700 W')
    axes[0, 1].set_title('(b) Source-sink balance', fontsize=11, fontweight='bold')
    ...
    # (d) [F] drop vs power
    axes[1, 1].plot(x, F_drop, 'r-o', lw=2.5, ms=7, label='Model')
    axes[1, 1].axhline(METTLER_FDROP_PCT, ls='--', color='green', lw=2,
                       label=f'Mettler ({METTLER_FDROP_PCT:.0f}%)')
    axes[1, 1].set_xlabel('RF Power (W)')
    axes[1, 1].set_ylabel('[F] drop (%)')
    axes[1, 1].set_title('(d) [F] drop vs power', fontsize=11, fontweight='bold')
...
def gen_residuals_stage10_style(power_data, mesh, inside, out_dir):
    """Two-panel residual diagnostics matching Stage 10 Fig 22.

    (a) Residual vs radius at 700 W — bar chart of (model - experiment) / experiment
        at five radial sampling points (0, 25, 50, 75, 100 mm).  Shade ±10 % band.
    (b) Model/Experiment ratio vs power — line plot of [F]_ICP_model / [F]_Mettler;
        shade ±factor-of-2 band (0.5-2.0) in light green.

    Mettler reference values are taken from the 700 W Fig. 4.5 cluster (~4e14 cm^-3
    at the ICP-source volume average), with a weakly increasing trend with power
    that we approximate linearly for the ratio plot.
    """
    # --- Panel (a): radial residual at 700 W ------------------------------
    ref = next((e for e in power_data if e['value'] == 700), None)
    if ref is None or 'F' not in ref['fields']:
        return
    nF = ref['fields']['F']
    ...
    # --- Panel (b): ratio vs power -----------------------------------------
    xp = [e['value'] for e in power_data]
    F_icp_model = np.array([e.get('summary', {}).get('F_icp', 0) for e in power_data])
    # Mettler reference: linear interpolation through (300 W, 2.6e14), (700 W, 4.0e14),
    # (1000 W, 4.8e14) — representative of Fig. 4.5 data scatter.
    mettler_power = np.array([300, 500, 700, 900, 1000])
    mettler_F = np.array([2.6e14, 3.4e14, 4.0e14, 4.6e14, 4.8e14])
    F_icp_mettler = np.interp(xp, mettler_power, mettler_F)
    ratio = F_icp_model / np.maximum(F_icp_mettler, 1e10)

    # --- Figure ------------------------------------------------------------
    fig, axes = plt.subplots(1, 2, figsize=(13, 5))

    ax = axes[0]
    ax.axhspan(-10, 10, color='#C6EFCE', alpha=0.55, label='$\\pm 10$% band')
    bar_colors = ['#2ca02c' if abs(v) <= 10 else '#d62728' for v in residual_pct]
    ...
    for r_val, res in zip(METTLER_RADIAL_R_MM, residual_pct):
        ...
        offset = 1.2 if res >= 0 else -1.2
        ax.text(r_val, res + offset, f'{res:+.1f}%', ha='center', va=va,
                fontsize=10, fontweight='bold',
                color=('#1a6e1a' if abs(res) <= 10 else '#8b0000'))
    ax.set_xlabel('$r$ (mm)', fontsize=13)
    ax.set_ylabel('Model error (%)', fontsize=13)
    ax.set_title('(a) Residual vs Radius at 700 W', fontsize=13, fontweight='bold')
    # Wide enough limits so residuals are fully visible (not saturated)
    ymax = max(30.0, np.abs(residual_pct).max() * 1.25)
    ax.set_ylim(-ymax, ymax)
    ax.legend(loc='upper left', fontsize=11)
    ax.grid(True, alpha=0.3, axis='y')

    ax = axes[1]
    ax.axhspan(0.5, 2.0, color='#C6EFCE', alpha=0.55, label='Factor-2 band')
    ax.plot(xp, ratio, '-o', color='#d62728', lw=2.2, ms=9, label='Model / Expt', zorder=3)
    ax.axhline(1.0, color='k', lw=0.9, ls='--')
    ax.set_xlabel('Power (W)', fontsize=13)
    ax.set_ylabel('Model / Experiment', fontsize=13)
    ax.set_title('(b) Model-to-Experiment Ratio vs Power', fontsize=13, fontweight='bold')
    ax.set_ylim(0, max(2.5, ratio.max() * 1.15))
    ax.legend(loc='upper right', fontsize=11)
    ax.grid(True, alpha=0.3)

    fig.suptitle('Model Residuals: Spatial and Power Dependence',
    ...
    plt.close(fig)
    print("  fig_residuals  (2-panel, Stage 10 Fig 22 style)")
