...
def gen_mettler_fig417_v2():
    ...
    # (c) residuals — y_lim set wide [-100, +40] to contextualise the -56 %
    # magnitude residual: it is a factor-of-two offset (well-defined deficit),
    # not a factor-of-10 failure. A cramped y-axis would make -56 % look like
    # a catastrophe; widening gives it the visual weight it deserves.
    ax = axes[2]
    ...
    # Acceptance contexts at multiple magnitude scales
    ...
    ax.axhspan(-50, -25, color='tab:orange', alpha=0.08)
    ax.axhspan(25, 50, color='tab:orange', alpha=0.08, label='$\\pm 50$% (factor-of-2)')
    ax.axhline(0, color='black', lw=1.0)
    ax.set_ylim(-100, 40)
    ax.set_xlabel('$r$ (cm)'); ax.set_ylabel('Model error vs Mettler (%)')
    ax.set_title('(c) Residuals at Mettler radii', fontweight='bold')
    ax.set_xticks(METTLER_R_CM); ax.legend(loc='lower left', fontsize=8, ncol=2)
    ax.grid(True, axis='y')

    fig.suptitle('Direct benchmark v2: Self-consistent Global–2D + m12 bias '
                 '(1000 W, 10 mTorr, 200 W bias) — residuals are composition-'
                 'dependent factor-of-2 offset (D6 FAIL: scalar γ$_\\mathrm{Al}$ cannot close)',
