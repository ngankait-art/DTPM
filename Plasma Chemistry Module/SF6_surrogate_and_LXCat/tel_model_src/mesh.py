...
class Mesh2D:
    ...
    def __init__(self, R, L, Nr, Nz, beta_r=1.3, beta_z=1.3):
        ...
        # Cell volumes: V = 2π r Δr Δz
        self.vol = np.zeros((Nr, Nz))
        for i in range(Nr):
            for j in range(Nz):
                self.vol[i, j] = 2 * np.pi * self.rc[i] * self.dr[i] * self.dz[j]

        # Face areas for flux computation
        # Radial faces: A_r = 2π r_face Δz
        # Axial faces: A_z = 2π r_c Δr (annular ring)
    ...
    def __repr__(self):
        return (f"Mesh2D({self.Nr}×{self.Nz}, R={self.R:.3f}m, L={self.L:.3f}m, "
                f"Δr_min={self.dr.min()*1e3:.2f}mm, Δz_min={self.dz.min()*1e3:.2f}mm)")


def make_icp_mesh(R=0.180, L=0.175, Nr=80, Nz=100, beta_r=1.3, beta_z=1.3):
    """Create a standard ICP reactor mesh (Lallement geometry)."""
    return Mesh2D(R, L, Nr, Nz, beta_r, beta_z)
