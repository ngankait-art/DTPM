"""
Boundary Conditions for TEL Reactor

Material-specific surface kinetics based on Kokkoris et al. (2009).
Each wall surface has distinct adsorption, recombination, and
SF6 regeneration probabilities.

References:
    Kokkoris et al., J. Phys. D 42, 055209 (2009) — Table 2
"""
...
def get_gamma_map(gamma_quartz=0.001, gamma_Al=0.18,
    ...
    ----------
    gamma_quartz : float
        F recombination probability on quartz.
    gamma_Al : float
        F recombination probability on aluminium.
    gamma_wafer : float
        F recombination probability on silicon.
    gamma_window : float
        F recombination probability on dielectric window.
