...
surrogate predicts.  Solid lines are the solver; dashed lines (with
markers) are the surrogate ensemble mean; the shaded bands are the
$\pm 1\sigma$ inter-model spread.  The surrogate tracks the solver
across the entire \SI{200}{\watt}--\SI{1200}{\watt} power range and
the 3--20~mTorr pressure range, with the
$\pm 1\sigma$ band narrow throughout.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{surrogate_vs_solver_neutrals_benchmark.png}
  \caption{Surrogate vs.\ solver benchmark for volume-averaged neutral
           densities.  (a) Power sweep at $p = 10~mTorr$,
           pure SF\textsubscript{6}.  (b) Pressure sweep at
           $P_\mathrm{rf} = \SI{700}{\watt}$, pure SF\textsubscript{6}.
           Solid: solver; dashed (with markers): surrogate ensemble
           mean; shaded band: ensemble $\pm 1\sigma$.  Surrogate
           outputs are nF and nSF\textsubscript{6}; charged-species
           prediction is outside the surrogate's two-output design
           and is not benchmarked here (Section~\ref{sec:limitations}).}
