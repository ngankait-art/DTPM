...
5-model production ensemble achieves nF RMSE $= 0.00381$ and
nSF\textsubscript{6} RMSE $= 0.00134$---a 66\% improvement on nF over
the v3 LXCat baseline and a $\sim$2$\times$ reduction on
nSF\textsubscript{6} relative to the legacy v4 surrogate, while
matching v4 on nF within $1.31\times$.  Ablation reveals output-bias
initialisation as the dominant factor (74\% of RMSE reduction).  The
surrogate delivers a $972\times$ speedup over the LXCat solver.
Limitations are discussed candidly, including a 36\% systematic error
in absolute F density versus published measurements.
\end{abstract}
...
\subsection{Surrogate vs.\ Solver at a Representative Operating Point}

Figure~\ref{fig:bench_overlay} overlays the radial F-density profile
at the wafer predicted by the production ensemble against the
fluid-solver ground truth at $P_\mathrm{rf} = 700$~W,
$p = 10$~mTorr, pure SF\textsubscript{6}.  The ensemble mean tracks
the solver across the entire radius; the $\pm 1\sigma$ inter-model
band is narrow and inflates only at the extremes where data is
sparser, consistent with the spatial residual map in
Figure~\ref{fig:residuals}.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{surrogate_vs_solver_overlay.png}
  \caption{Surrogate ensemble mean ($\pm 1\sigma$) versus solver radial
           F-density profile at the wafer for the representative
           operating point P\textsubscript{rf}~=~\SI{700}{\watt},
...
\begin{enumerate}
  ...
  \item \textbf{Supervised surrogates succeed.}  A systematic
        ...
        ensemble (nF RMSE = 0.00381) achieved a $35\times$ error
        reduction.  The E3\_separate\_heads architecture wins a
        7-experiment, 3-seed sweep.  Output-bias initialisation is the
        dominant training factor (74\% of RMSE reduction);
        regularisation without it is useless.  The 5-model production
        ensemble achieves nF RMSE = 0.00381 and nSF\textsubscript{6}
        RMSE = 0.00134---a 66\% improvement on nF over the v3 LXCat
        baseline and a $\sim$2$\times$ reduction on
        nSF\textsubscript{6} relative to the legacy v4 surrogate,
        while matching v4 on nF within $1.31\times$, with a
        $524\times$/$972\times$ speedup over the legacy/LXCat solvers.
