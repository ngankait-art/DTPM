...
def ode_rhs(t, y, p_OFF, P_abs):
    ...
    State vector y = [n_SF6, ..., n_e (13 species), Te (eV), θ_F, θ_SF3, θ_SF4, θ_SF5]
    Total: 18 ODEs.
    """
    ...
    # --- Ion wall losses (full Lee & Lieberman with electronegative correction) ---
    alpha_avg = (n[Fm] + n[SF6m]) / max(ne, 1e6)
    wSF5p = ion_wall_freq(SF5p, Te, ng, alpha_avg, p_OFF)
    wSF4p = ion_wall_freq(SF4p, Te, ng, alpha_avg, p_OFF)
    wSF3p = ion_wall_freq(SF3p, Te, ng, alpha_avg, p_OFF)
    wF2p  = ion_wall_freq(F2p,  Te, ng, alpha_avg, p_OFF)
    ...
    eta_power = 0.50  # ICP coupling efficiency
