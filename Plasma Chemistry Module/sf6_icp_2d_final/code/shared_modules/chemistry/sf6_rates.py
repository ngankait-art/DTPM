...
if __name__ == '__main__':
    print("Chemistry module self-test")
    print("=" * 50)

    # Reference conditions from 0D model: 1500W, 10 mTorr, pure SF6
    Te_ref = 3.0  # eV
    k = rates(Te_ref)

    print(f"\nRate coefficients at Te = {Te_ref} eV:")
    print(f"  iz_SF6_total = {k['iz_SF6_total']:.3e} m³/s")
    print(f"  att_SF6_total = {k['att_SF6_total']:.3e} m³/s")
    print(f"  d1 (SF6→SF5+F) = {k['d1']:.3e} m³/s")
    print(f"  Ar_iz = {k['Ar_iz']:.3e} m³/s")
    print(f"  Ar_exc = {k['Ar_exc']:.3e} m³/s")
    print(f"  Ar_iz_m = {k['Ar_iz_m']:.3e} m³/s")
    print(f"  Penn_SF6 = {k['Penn_SF6']:.3e} m³/s")

    # Check Troe rates at 10 mTorr
    from scipy.constants import k as kB
    ng = 0.133322 * 10 / (kB * 300)  # 10 mTorr, 300K → m⁻³
    k = compute_troe_rates(k, ng)
    print(f"\nTroe rates at 10 mTorr (ng = {ng:.2e} m⁻³):")
    print(f"  nr42 (SF5+F→SF6) = {k['nr42']:.3e} m³/s")
    print(f"  nr41 (SF4+F→SF5) = {k['nr41']:.3e} m³/s")
    print(f"  nr40 (SF3+F→SF4) = {k['nr40']:.3e} m³/s")

    # Check electron source at reference densities
    ne = 6e15; nSF6 = 1e19; nSF5 = 2e17; nSF3 = 5e16
    nF = 1.2e20; nS = 1e15; nAr = 0; nArm = 0
    Se = electron_source(k, ne, nSF6, nSF5, nSF3, nF, nS, nAr, nArm)
    print(f"\nElectron source at reference conditions:")
    print(f"  Se = {Se:.3e} m⁻³/s")
    print(f"  Ionization freq = {Se/ne:.1f} s⁻¹")
